class Pillars::CreatePillarTopicsJob < ApplicationJob
  ...
  def create_pillar_topic(pillar_column:, pillar_topic:)
    ...
    Rails.logger.warn "Failed to create PillarTopic for PillarColumn #{pillar_column.id} with title '#{pillar_topic["title"]}' and summary '#{pillar_topic["summary"]}': #{e.message}"
  end
end
