...
RSpec.describe Stories::CreateStoriesJob, type: :job do
  describe '#perform' do
    ...
    let(:feed) { create(:feed, sub_topic:) }
    let!(:tag1) { create(:tag, name: 'tag1') }
    let!(:tag2) { create(:tag, name: 'tag2') }
    let!(:tag3) { create(:tag, name: 'tag3') }
    let!(:feed_items) do
      [
        create(:feed_item, feed:, tags: [tag1, tag2]),
        create(:feed_item, feed:, tags: [tag1, tag2, tag3]),
        create(:feed_item, feed:, tags: [tag1, tag2, tag3]),
        create(:feed_item, feed:, tags: [tag1, tag2, tag3]),
        create(:feed_item, feed:, tags: [tag1, tag2]),
        create(:feed_item, feed:, tags: [tag1, tag3])
      ]
    end

    subject(:job) { described_class.perform_now(sub_topic:) }
